...
function checkLayoutCollisions() {
	...
	try {
		...
		const isNarrowLandscape = window.matchMedia(
		...
		if (header && header.offsetParent !== null) {
			...
			if (center && center.offsetParent !== null) {
				...
				const navButtons = Array.from(center.querySelectorAll("button")).reduce(
					(sum, button) => {
				...
				fixedWidth +=
					navButtons +
					navGap * Math.max(0, center.querySelectorAll("button").length - 1) +
					minTitleWidth +
					parseFloat(style.marginLeft || 0) +
					parseFloat(style.marginRight || 0);
			}
			...
			const headerGap = parseFloat(
				headerStyle.gap || headerStyle.columnGap || 0,
			);
			...
			const visualHeaderWidth =
				header.getBoundingClientRect().width || header.clientWidth;
		...
		if (layoutFits && footer && footer.offsetParent !== null) {
			...
			const footerGap = parseFloat(
				footerStyle.gap || footerStyle.columnGap || 0,
			);
			footerRequiredWidth += footerGap * 2;

			const visualFooterWidth =
				footer.getBoundingClientRect().width || footer.clientWidth;
...
function isCollapsibleLayout() {
	...
	if (
		window.matchMedia("(max-width: 1100px) and (orientation: landscape)")
			.matches
	) {
		return true;
	}
...
function onToggleFamilyChordDropdown(event) {
	...
	document
		.querySelectorAll(".family-chord-dropdown")
...
function onGlobalDocumentClick(event) {
	...
	if (
		!event.target.closest(".midi-collapse") &&
		typeof midiPanel !== "undefined" &&
		midiPanel
	) {
		if (typeof midiToggleBtn !== "undefined" && midiToggleBtn)
...
function setupRippleEffect() {
	...
	document.body.addEventListener("click", (e) => {
		const rippleTarget = e.target.closest(
			".nav-btn, .icon-button, .pujian-list li, .accent-color, .list-action-btn, .mini-player-info",
		);
		if (rippleTarget) {
			createRipple({
				currentTarget: rippleTarget,
				clientX: e.clientX,
				clientY: e.clientY,
			});
		}
...
function checkOrientation() {
	...
	orientationWarning.classList.toggle(
		"visible",
		currentViewMode === "double" && isPortrait,
	);
}
...
function rebuildInstrumentSelectors(sfUrl) {
	var sfKey =
		typeof normalizeSoundfontKey === "function"
			? normalizeSoundfontKey(
					sfUrl || (prefs && prefs.midiSoundfont) || MIDI_SF2_URL,
				)
			: sfUrl || (prefs && prefs.midiSoundfont) || MIDI_SF2_URL;
	var preferredProgram =
		typeof prefs !== "undefined" && prefs && prefs.midiInstrument != null
			? String(prefs.midiInstrument)
			: "";
	var built =
		typeof buildSoundfontInstrumentOptionsHtml === "function"
			? buildSoundfontInstrumentOptionsHtml(sfKey, preferredProgram)
			: {
					html: "",
					list: [],
					activeProgram: preferredProgram,
					activeLabel: "Memuat Instrumen...",
				};
	...
	if (typeof prefs !== "undefined") {
		if (
			...
			typeof prefs.midiInstrumentBySoundfont !== "object"
		) {
			prefs.midiInstrumentBySoundfont = {};
		}
		if (
			String(prefs.midiInstrument) !== currentVal ||
	...
	var iconText =
		typeof getMidiInstrumentIcon === "function"
...
function handleTouchStart(event) {
	...
	const baseScale =
		typeof currentScale === "number" && Number.isFinite(currentScale)
			? currentScale
			: initialScale;
...
function handleMainContentClick(e) {
	...
	if (uiStyleButton) {
		...
		const activeStyleLabel = document.getElementById(
			"settings-active-style-label",
		);
		const activeStyleMeta = UI_STYLE_PRESETS.find(
			(preset) => preset.key === appliedStyle,
		);
	...
	if (layoutStyleButton) {
		...
		const activeLayoutLabel = document.getElementById(
			"settings-active-layout-label",
		);
		const activeLayoutMeta = LAYOUT_STYLE_PRESETS.find(
			(preset) => preset.key === appliedLayout,
		);
	...
	if (colorSchemeButton) {
		...
		const activeSchemeLabel = document.getElementById(
			"settings-active-scheme-label",
		);
		const activeSchemeMeta = COLOR_SCHEME_PRESETS.find(
			(preset) => preset.key === appliedScheme,
		);
	...
	if (settingsDropdownOption) {
		...
		if (wrapper) {
			...
			if (label) {
				...
				label.textContent = settingsDropdownOption.textContent
					.replace(/^check\s*/, "")
					.trim();
			}
	...
	if (settingsDropdownBtn) {
		...
		if (wrapper) {
			...
			document
				.querySelectorAll(".settings-custom-dropdown.is-open")
				.forEach((other) => {
					if (other !== wrapper) {
			...
			settingsDropdownBtn.setAttribute(
				"aria-expanded",
				isOpen ? "true" : "false",
			);
		}
	...
	if (pujianItem) {
		if (
			pujianItem.closest("#playlist-track-list") ||
			pujianItem.classList.contains("playlist-track-item")
		)
			return;
		if (e.target.closest(".add-to-playlist-btn") || e.target.closest("button"))
			return;
	...
	if (chordThemeButton) {
		...
		persistChordUiPrefs();
		chordThemeButton.parentElement
			.querySelector(".selected")
			?.classList.remove("selected");
	...
	if (chordFillColorButton) {
		...
		persistChordUiPrefs();
		chordFillColorButton.parentElement
			.querySelector(".selected")
			?.classList.remove("selected");
...
function syncHeaderBranding() {
	...
	if (
		!logoElement ||
		!brandElement ||
		typeof HEADER_LOGO_VARIANTS !== "object"
	) {
		return;
	}

	const activeStyle =
		UI_STYLE_PRESETS.find(
			(preset) => preset.key === document.body.getAttribute("data-ui-style"),
		) || UI_STYLE_PRESETS[0];
	const variantKey = isEffectiveDarkTheme()
		? "white"
		: activeStyle?.logoVariant || "color";
	const nextLogo =
		HEADER_LOGO_VARIANTS[variantKey] || HEADER_LOGO_VARIANTS.color;
...
function handleSettingsChange(e) {
	...
	if (targetId === "dark-theme-toggle") {
		...
		if (
			...
			typeof MidiEngine.setPreloadCacheMax === "function"
		) {
		...
		if (typeof resolveSoundfontInstrumentProgram === "function") {
			prefs.midiInstrument = resolveSoundfontInstrumentProgram(
				nextSf,
				prefs.midiInstrumentBySoundfont[nextSf] || "",
			);
		}
		...
		document
			...
			.forEach((wrapper) => {
				wrapper.classList.remove("is-open");
				var capBtn = wrapper.querySelector(
		...
		if (palette) {
			if (chordUiPrefs.syncThemeWithAccent)
				palette.classList.add("is-disabled");
		...
		const customAccentBtn = document.querySelector(
			'.accent-color[data-color="custom"]',
		);
...
function applyStoredPreferences() {
	...
	customAccentColor =
		localStorage.getItem(ACCENT_CUSTOM_COLOR_KEY) || DEFAULT_CUSTOM_ACCENT;
	document.documentElement.style.setProperty(
		"--source-custom",
		customAccentColor,
	);
	...
	try {
		...
		const validColorSchemeKeys = new Set(
			COLOR_SCHEME_PRESETS.map((item) => item.key),
		);
		const validUiStyleKeys = new Set(UI_STYLE_PRESETS.map((item) => item.key));
		const validLayoutStyleKeys = new Set(
			LAYOUT_STYLE_PRESETS.map((item) => item.key),
		);
		...
		if (
			!Number.isFinite(Number(prefs.preloadCacheMax)) ||
			Number(prefs.preloadCacheMax) < 1
		) {
		...
		if (prefs.midiSoundfont) {
			...
			prefs.midiInstrumentUserSelected =
				String(prefs.midiInstrument || "") !== "";
			localStorage.setItem("prefs", JSON.stringify(prefs));
		}
		if (
		...
		applyLayoutStyleSelection(
			LAYOUT_STYLE_PRESETS[0]?.key || "balanced",
			false,
		);
	}
	...
	try {
		...
		if (storedChordUi) {
			...
			const validThemeKeys = new Set(
				CHORD_THEME_PRESETS.map((item) => item.key),
			);
			const validFillKeys = new Set(CHORD_FILL_PRESETS.map((item) => item.key));
			chordUiPrefs = {
				...chordUiPrefs,
				...parsed,
				theme: validThemeKeys.has(parsed.theme)
					? parsed.theme
					: chordUiPrefs.theme,
				fillColor: validFillKeys.has(parsed.fillColor)
					? parsed.fillColor
					: chordUiPrefs.fillColor,
