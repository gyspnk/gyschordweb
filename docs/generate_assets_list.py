...
NOTE_CHORD_OUTPUT_FILE = os.path.join(os.path.dirname(__file__), 'note-chord-assets-list.json')
...
def list_note_chord_jsons():
    if not os.path.exists(ASSETS_CHORD_DIR): return []
    return sorted([
        f for f in os.listdir(ASSETS_CHORD_DIR)
        if f.lower().endswith('.chord.json') and os.path.isfile(os.path.join(ASSETS_CHORD_DIR, f))
    ])

def main():
    pdfs = list_pdfs()
    chords = list_chord_txts()
    note_chords = list_note_chord_jsons()
    ...
    with open(NOTE_CHORD_OUTPUT_FILE, 'w', encoding='utf-8') as f:
        json.dump(note_chords, f, ensure_ascii=False, indent=2)
